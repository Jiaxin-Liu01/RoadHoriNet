...
from horizenLine.utils.general import check_requirements, xyxy2xywh, xywh2xyxy, xywhn2xyxy, xyn2xy, segment2box, segments2boxes, \
    resample_segments, clean_str
from horizenLine.utils.torch_utils import torch_distributed_zero_first
...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=True, scaleFill=False, scaleup=True, stride=32):
    ...
    # 将给定的图像 img 调整（缩放和填充）到一个新的形状 new_shape，同时确保调整后的图像尺寸满足特定的步长 stride 的倍数要求
    # 返回调整并填充后的图像、宽度和高度方向的缩放比例 ratio、填充的宽度 dw 和高度 dh。
...
def random_perspective(img, targets=(), segments=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0,
    ...
    # targets = [cls, xyxy]
    '''
    数据增强：对图像及其目标（如边界框或分割区域）应用随机透视变换。这种变换可以包括旋转、缩放、剪切、平移和透视变形，以模拟图像在不同视角下的外观
    img: 输入的图像，假设是一个NumPy数组，形状为(h, w, c)，其中h是高度，w是宽度，c是通道数。
    targets: 一个包含目标信息的元组或列表，每个目标通常是一个包含类别和边界框（xyxy格式）的列表或数组。
    segments: 一个包含分割区域信息的列表，每个区域由一系列的点组成。
    degrees: 旋转角度的范围（度）。
    translate: 平移的比例。
    scale: 缩放的比例。
    shear: 剪切的角度范围（度）。
    perspective: 透视变形的强度。
    border: 边界填充的大小，用于在变换时保持图像大小不变。'''
