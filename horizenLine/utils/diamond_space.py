...
def heatmap_to_vp(vp_heatmap, res, scale=1.0):
    Rinv = np.linalg.inv(np.array([[1, -1], [1, 1]]))  # 获取矩阵的逆
    vp_diamond = Rinv @ (2 / (res - 1) * vp_heatmap - 1.0)
    vp_scaled = original_coords_from_diamond(vp_diamond, 1.0)  # 钻石空间的原始坐标
    vp = vp_scaled / scale
    return vp
...
def heatmap_to_orig(res, scale=1.0):
    heatmap_orig = np.empty([res, res, 2], dtype=np.float32)
    for i in range(res):
        for j in range(res):
            heatmap_orig[i, j] = heatmap_to_vp(np.array([i, j], dtype=np.float32), res, scale)
...
def process_heatmap(heatmap, scale):
    ...
    heatmap_orig_x = heatmap_orig[:, :, 0][
        ~np.logical_or(np.isinf(heatmap_orig[:, :, 0]), np.isnan(heatmap_orig[:, :, 0]))]
    heatmap_orig_y = heatmap_orig[:, :, 1][
        ~np.logical_or(np.isinf(heatmap_orig[:, :, 1]), np.isnan(heatmap_orig[:, :, 1]))]
...
# ---------------------------------------------------------------------------------------------------------------------#
# 旨在从热力图中找到一个主要的视点（vp_max），并计算该视点到其他高响应区域（即响应值大于热力图最大值的80%的区域）的平均归一化距离
# ---------------------------------------------------------------------------------------------------------------------#
...
# --------------------------------------------------------------------------------------------------------------------#
# 处理一系列的热力图，
# 这些热力图包含了在不同尺度（scales）下的关键点信息。
# 对于每个尺度和每个样本，从对应的热力图中提取出两个视点（viewpoint）的位置（vp1 和 vp2）以及它们与真实位置的距离（vp1_dist 和 vp2_dist）
# --------------------------------------------------------------------------------------------------------------------#
