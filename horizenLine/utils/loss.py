...
def focal_loss(pred, target, alpha=2, gamma=4):
    # -------------------------------------------------------------------------#
    #   找到每张图片的正样本和负样本
    #   除去正样本的特征点，其余为负样本
    # -------------------------------------------------------------------------#
    ...
    # -------------------------------------------------------------------------#
    pos_loss = alpha * torch.pow(1 - pred, gamma) * torch.log(pred) * pos_inds
    neg_loss = alpha * torch.pow(pred, gamma) * torch.log(1 - pred) * neg_inds
...
from horizenLine.utils.general import bbox_iou, bbox_alpha_iou, box_iou, box_giou, box_diou, box_ciou, xywh2xyxy
from horizenLine.utils.torch_utils import is_parallel
